...
from sound_module import play_mp3_file, transcribe_file_v2, google_tts, start, stop
...
if st.session_state['chat_history'] :
    start("C:/Users/SBA/Desktop/Proj/input.wav")
    time.sleep(1)  # 사용자 음성을 8초간 녹음
    stop()
...
step 8. 모든 대화가 끝나면 '이용해 주셔서 감사합니다. 다음에 또 오세요.'라고 출력한다.
</지시사항>
...
# sentence를 계속 불러와야 하므로 하드코딩보단 for문 이용
        ...
        for i in range(len(sentences)):
            ...
            if ("주문하" in sentences[i]) or ("추가" in sentences[i]):
                    ...
                    for j in range(len(splited)) : 
                        for k in itemlist :
                            if k in splited[j]:
                                num = re.sub(r'[^0-9]', '', splited[j])
                                buy_list.append(k)
                                buy_list.append(num)
                                print(int(메뉴[k]))
                                print(int(num))
                                total_price = total_price + 메뉴[k]*int(num)
                                
            if "총 가격" in sentences[i]:
                print(buy_list)
                print(total_price)
                sentences[i] = " 총 가격은 "+ str(total_price) +"원입니다"
    ...
    if "이용해 주셔서 감사합니다. 다음에 또 오세요." in response:
        for i in range(len(st.session_state['buy_list'])//2):
           now = datetime.now()
           cur.execute("INSERT INTO `kiosk_db`.`sale_test` (`{}`, `{}`, `{}`, `{}`) VALUES ('{}', '{}', '{}', '{}')".format("sale_date", "item_name", "item_price", "item_amount", now, st.session_state['buy_list'][2*i], 메뉴[st.session_state['buy_list'][2*i]], st.session_state['buy_list'][2*i+1]))
           # cur.execute("insert into 'kiosk_db'.'sale_test'({},{},{})".format(st.session_state['buy_list'][2*i],메뉴[st.session_state['buy_list'][2*i]],st.session_state['buy_list'][2*i+1]))
           cur.execute("commit")
        del st.session_state['chat_history']
        del st.session_state['buy_list']
