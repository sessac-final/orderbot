...
for i in range(len(st.session_state['chat_history'])//2):
    if i!=0:
        ctext.append(InputOutputTextPair(
            input_text=st.session_state['chat_history'][2*i][1],
            output_text=st.session_state['chat_history'][2*i+1][1]))
...
chat = model.start_chat(context=context, examples=ctext)
